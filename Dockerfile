...
RUN echo '[supervisord]' > /etc/supervisor/conf.d/supervisord.conf && \
    ...
    echo 'command=gunicorn --logger-class gunicorn_config.CustomLogger --config gunicorn_config.py --preload --bind 0.0.0.0:5000 --workers 2 --timeout 600 app:app' >> /etc/supervisor/conf.d/supervisord.conf && \
