...
- Приоритет: `webhook_headers` из запроса > глобальные `WEBHOOK_HEADERS`
...
| **Конфигурация Redis** |||
| ~~`REDIS_HOST`~~ | `localhost` | ❌ **Не настраивается** в публичной версии. Встроенный Redis. |
| ~~`REDIS_PORT`~~ | `6379` | ❌ **Не настраивается** в публичной версии. Встроенный Redis. |
| ~~`REDIS_DB`~~ | `0` | ❌ **Не настраивается** в публичной версии. Встроенный Redis. |
| **Управление задачами** |||
| ~~`CLEANUP_TTL_SECONDS`~~ | `86400` | ❌ **Не настраивается** в публичной версии. Фиксировано 24 часа. |
| **Конфигурация Webhook** |||
| `WEBHOOK_RETRY_ATTEMPTS` | `3` | Максимум попыток доставки webhook (немедленные повторы при ошибке). |
| `WEBHOOK_RETRY_INTERVAL_SECONDS` | `5` | Задержка между немедленными повторами webhook (секунды). |
| `WEBHOOK_TIMEOUT_SECONDS` | `8` | Таймаут запроса webhook (секунды). |
| ~~`WEBHOOK_BACKGROUND_INTERVAL_SECONDS`~~ | `900` | ❌ **Не настраивается** в публичной версии. Фоновый resender сканирует каждые 15 минут (фиксировано). |
| `DEFAULT_WEBHOOK_URL` | — | Резервный URL webhook (async). Используется когда запрос не содержит `webhook_url`. Должен начинаться с http(s)://, < 2048 символов. |
| `WEBHOOK_HEADERS` | — | Дополнительные заголовки для webhook POST. JSON объект или список: `{"Authorization":"Bearer XXX","X-Source":"ytdl"}` ИЛИ `Authorization: Bearer XXX; X-Source: ytdl`. Чувствительные заголовки (Authorization, X-API-Key, X-Auth-Token) маскируются в логах запуска. |
| **Логирование** |||
...
| **Лимиты метаданных клиента** |||
| `MAX_CLIENT_META_BYTES` | `16384` | Макс. размер для `client_meta` (байты). |
| `MAX_CLIENT_META_DEPTH` | `5` | Макс. глубина вложенности для `client_meta`. |
...
- URL webhook сохраняется в метаданных задачи (поле `webhook_url` в ответе `/task/{task_id}`)
...
**Конфигурация:**
- Используйте `DEFAULT_WEBHOOK_URL` для установки fallback webhook для всех async задач без явного `webhook_url`
- Используйте `WEBHOOK_HEADERS` для добавления заголовков аутентификации: `{"Authorization": "Bearer XXX"}`
- Мониторьте доставку webhook в логах (установите `LOG_LEVEL=DEBUG` для детального просмотра payload)

**Пример с default webhook:**
```yaml
environment:
