...
| ~~`WEBHOOK_RETRY_ATTEMPTS`~~ | `3` | ⚙️ **Зашит в коде** в публичной версии. Максимум попыток доставки webhook (немедленные повторы при ошибке). Настраивается в Pro. |
| ~~`WEBHOOK_RETRY_INTERVAL_SECONDS`~~ | `5` | ⚙️ **Зашит в коде** в публичной версии. Задержка между немедленными повторами webhook (секунды). Настраивается в Pro. |
| ~~`WEBHOOK_TIMEOUT_SECONDS`~~ | `8` | ⚙️ **Зашит в коде** в публичной версии. Таймаут запроса webhook (секунды). Настраивается в Pro. |
| ~~`WEBHOOK_BACKGROUND_INTERVAL_SECONDS`~~ | `900` | ⚙️ **Зашит в коде** в публичной версии. Фоновый resender сканирует каждые 15 минут (фиксировано). Настраивается в Pro. |
| ~~`DEFAULT_WEBHOOK_URL`~~ | — | ❌ **Недоступен** в публичной версии. Указывайте `webhook.url` в каждом запросе. Доступен в Pro версии. |
...
| ~~`MAX_CLIENT_META_KEYS`~~ | `200` | ⚙️ **Зашит в коде** в публичной версии. Макс. количество ключей в объекте `client_meta`. Настраивается в Pro. |
| ~~`MAX_CLIENT_META_STRING_LENGTH`~~ | `1000` | ⚙️ **Зашит в коде** в публичной версии. Макс. длина строковых значений. Настраивается в Pro. |
| ~~`MAX_CLIENT_META_LIST_LENGTH`~~ | `200` | ⚙️ **Зашит в коде** в публичной версии. Макс. длина массивов. Настраивается в Pro. |
